...
assign CPU_TAn_o        = CPU_TAn;
assign CPU_TEAn_o 		= CS_NO_MANSLAND ? CPU_TAn : 1'b1;
assign CPU_TCIn_o 		= CS_None_Cacheable ? CPU_TAn : 1'b1;
assign CPU_TBIn_o		= LineTransfer ? 1'b1 :  CPU_SIZ_i[1:0] == 2'b11  ? CPU_TAn : 1'b1 ; // So right now, LINE16 is allowed only in SRAM
assign CPU_AVECn_o 		= (iIRQ_AutoVector_i & CS_GET_VECTOR_INT ) ? CPU_TAn : 1'b1;	// AVEC is the signal that terminate the cycle when the AutoVector is triggered
...
assign LOCAL_MEM_FLASH_OEn_o 	= TSF_FLASH2RAM_o ? (Dbg_Mode_On_i ? Dbg_FLASH_OE_i 	: !CS_FLASH ) : TSF_FLASH_OE;
...
always @ (*) begin
	if ( TSF_FLASH2RAM_o ) begin
			casex ( { Dbg_Mode_On_i, LineA3_A2_Switch} )
			2'b00: begin
				LOCAL_MEM_SRAM_WEn_o = !(!CPU_RWn_i & !CPU_TAn & CS_SRAM & !CPU_BCLK_o);  // Just Do the Write when the Clock is LOW
