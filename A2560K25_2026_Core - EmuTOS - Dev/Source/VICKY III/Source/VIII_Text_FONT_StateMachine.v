`timescale 1ns / 1ps
module VIII_Text_FONT_ST_ChannelA #(
//---------------------------Parameters----------------------------------------
...
always @ (*)
begin
	if ((X_Char_Count == Cursor_X_Input_i[6:0]) && (Y_Char_Count == Cursor_Y_Input_i[5:0]) && FlashOnOFF && Cursor_Control_Reg_i[0])
...
assign 	ASCII_Text_Addy  	= ASCII_Text_Addy_Ptr[13:0] +  {6'b00_0000, X_Char_Count[6:0]};
...
//			if ( Vsync_EDGE == 2'b01 ) begin		// Bit Zero is the Enable Bit
		...
		begin
			if (Height_Pointer < FONT_Height_Constant) begin
				...
				Y_Char_Count <= Y_Char_Count + 6'h01;				
