...
output	wire				Mstr_Ctrl_Reset_Pll_o,
output	wire	[1:0]		Mstr_Ctrl_Video_Mode_o,
output	wire				Mstr_Ctrl_Doubling_Pixel_o,
output	wire				Mstr_Ctrl_Video_Mode_AUX_o, 


output	wire	[1:0]		Mstr_Ctrl_Video_Mode_PLL_o,
output	wire				Mstr_Ctrl_Doubling_Pixel_PLL_o,
output	wire				Mstr_Ctrl_Video_Mode_PLL_Aux_o,
...
output	wire				Mstr_Ctrl_Doubling_Pixel_100MhzResynced_o
);
/*
wire [7:0] Source;
wire [31:0] Probe;
...
begin
	...
	begin
		...
		VICKY_MASTER_REG[1] 	<= 32'h0000_0000;		// Border Control Reg 0
		VICKY_MASTER_REG[2] 	<= 32'h0000_0000;		// Border Control Reg 2
...
begin
	case(BUS_A_i[5:2])
		4'b0000: BUS_D_o = { PLL_Active_Clock_i, DIPSwitch_HiRes_i, DIPSwitch_GAMMA_i, VICKY_MASTER_REG[0][28:0] };
...
assign Mstr_Ctrl_Text_Mode_Enable_o 			= Master_Control_Reg_VidClk[2][0];
assign Mstr_Ctrl_Text_Mode_Overlay_o 			= Master_Control_Reg_VidClk[2][1];
assign Mstr_Ctrl_Graphic_Mode_Enable_o 		= Master_Control_Reg_VidClk[2][2];
assign Mstr_Ctrl_Video_Mode_o[1:0] 				= Master_Control_Reg_VidClk[2][9:8]; // Video Clock Synced
assign Mstr_Ctrl_Doubling_Pixel_o				= Master_Control_Reg_VidClk[2][10]; 	// Pixel Doubling
assign Mstr_Ctrl_Video_Mode_AUX_o		 		= Master_Control_Reg_VidClk[2][11]; // Video Clock Synced

assign Mstr_Ctrl_GAMMA_Enable_o 					= Master_Control_Reg_VidClk[2][16] ? Master_Control_Reg_VidClk[2][17] : DIPSwitch_GAMMA_i; 	//Master_Control_Reg_VidClk[2][17] : DIPSwitch_GAMMA_i;
assign Mstr_Ctrl_Turn_Off_Sync_o 				= !Master_Control_Reg_VidClk[2][18];		// Turn-Off Sync to get the Monitor to Sleep
...
begin
	...
	ReSync_VideoMode[0][2:0]<= VICKY_MASTER_REG[0][10:8];
	ReSync_VideoMode[1][2:0]<= ReSync_VideoMode[0][2:0];
	if ( ReSync_VideoMode[1][2:0]  == ReSync_VideoMode[0][2:0] )
		ReSync_VideoMode[2][2:0] <= ReSync_VideoMode[1][2:0];
end

assign Mstr_Ctrl_Bitmap_Enable_o = ReSync_Bitmap_Enable[2];
assign Mstr_Ctrl_TileMap_Enable_o = ReSync_TileMap_Enable[2];
assign Mstr_Ctrl_Sprite_Enable_o = ReSync_Sprite_Enable[2];
assign Mstr_Ctrl_Disable_Video_o = ReSync_DisableVideo[2];
assign Mstr_Ctrl_Video_Mode_100MhzReSynced_o[1:0] 	= ReSync_VideoMode[2][1:0]; // VICKY II Hi-Speed Synced
assign Mstr_Ctrl_Doubling_Pixel_100MhzResynced_o   = ReSync_VideoMode[2][2];

reg	[31:0]	VICKY_MASTER_REG1_RESYNC_AUX[0:2];

always @ (posedge Aux_Clk_i) 
begin              
		VICKY_MASTER_REG1_RESYNC_AUX[0] <= VICKY_MASTER_REG[0];
		VICKY_MASTER_REG1_RESYNC_AUX[1] <= VICKY_MASTER_REG1_RESYNC_AUX[0];
		if ( VICKY_MASTER_REG1_RESYNC_AUX[1] == VICKY_MASTER_REG1_RESYNC_AUX[0] ) 
			VICKY_MASTER_REG1_RESYNC_AUX[2] <= VICKY_MASTER_REG1_RESYNC_AUX[1];
end 

assign Mstr_Ctrl_Video_Mode_PLL_Aux_o = VICKY_MASTER_REG1_RESYNC_AUX[2][11];

assign Mstr_Ctrl_Doubling_Pixel_PLL_o = VICKY_MASTER_REG1_RESYNC_AUX[2][10];
assign Mstr_Ctrl_Video_Mode_PLL_o = VICKY_MASTER_REG1_RESYNC_AUX[2][9:8];

assign Mstr_Ctrl_Reset_Pll_o = VICKY_MASTER_REG1_RESYNC_AUX[2][7];
