...
// JOYSTICK 0
output	wire					JOY0_BTN0_io,		// RESET
output	wire					JOY0_BTN1_io,		// CLK
output	wire					JOY0_BTN2_io,		// CS
input		wire					JOY0_UP_io,			// MISO
output	wire					JOY0_DWN_io,		// MOSI
input		wire					JOY0_LFT_io,		// LINKn
input		wire					JOY0_RGHT_io,		// INT
output	wire					JOYSTICK0_RLY_o,
// JOYSTICK 1
inout		wire					JOY1_BTN0_io,		//
inout		wire					JOY1_BTN1_io,		//
inout		wire					JOY1_BTN2_io,		// 
input		wire					JOY1_UP_io,			// 
input		wire					JOY1_DWN_io,		// 
input		wire					JOY1_LFT_io,		//
input		wire					JOY1_RGHT_io,		//
...
/// INTERNAL COLD RESET Countdown
reg [23:0]	Countdown_ColdReset;
initial Countdown_ColdReset = 24'hFF_FFFF;
wire InternalColdReset;


always @ ( posedge Clk24 ) begin 
	if ( PLL_SDcard_Locked ) begin 
		if ( Countdown_ColdReset ) begin
			Countdown_ColdReset <= Countdown_ColdReset - 24'd1;
		end
		else begin
		  if ( Manual_RESET )
		      Countdown_ColdReset <= 24'hFF_FFFF;
		end
	end 
end 
assign InternalColdReset = ( Countdown_ColdReset ) ? 1'b1 : 1'b0; 
...
BIDIR_SIGNAL	RESET_BUFFER (
	...
	.oe ( InternalColdReset )	
	);


assign AUD_PDn_o = COLD_RESETn_In;
assign Ext_Reset = !COLD_RESETn_In;
...
begin
	Clk7_14Mhz <= Clk7_14Mhz ^ 1'b1;
end

// Clock Divide
always @ (posedge Clk7_14Mhz)
begin
	Clk3_58Mhz <= Clk3_58Mhz ^ 1'b1;
end




assign CPU_PCLK_o = Clk160;
...
PLL_SDCard_Debug	PLL_SDCard_Debug_inst (
	...
	.c0 ( Clk24 ),		// Output on the Processor Clock
	.c2 ( Clk160 ),
	.c3 ( Clk192 ),
	.c4 ( Clk096 ),
...
MC68040V_Interface M68040V(

	.Global_Reset_i( Ext_Reset ),
	.Init_F2R_TSF_i( Ext_Reset ), 
	.Clk_48Mhz_i( 1'b0 ),			// 50Mhz
	.Clk_24Mhz_i( OSC_CLK_25_175Mhz_B_i ), 
	
	.CPU_Clk_o( CPU_Clk ),					// 25Mhz Clock
...
// Reset
	...
	.SDRAM_Clk_i( Clk160 ), 	// CPU Clock is right now 40/2, so they are in Sync, no FIFO needed.
// Buses
	.iBUS_A_i( iBUS_A ),
...
DebugModule GavinDebug(
	...
	.Serial_Clk_i( Clk24 ),
	.Serial_Rst_i( Ext_Reset ),
	.System_Rst_i( Ext_Reset ),
	// Debug Interface
	.Dbg_Clk_i( CPU_Clk  ),
