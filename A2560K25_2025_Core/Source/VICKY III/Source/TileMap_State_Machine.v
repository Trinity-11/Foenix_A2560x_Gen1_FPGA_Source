...
always @ (*) begin
	case({Mstr_Ctrl_Doubling_Pixel_100Mhz_i, Mstr_Ctrl_Video_Mode100Mhz_i[1:0]})
		3'b000: MaxValueTileMap2BeRead = 8'd22;	// (42) 16 + 640 + 16 ( 1 + 40 + 1) Y: 480 + 32 = 32 Tiles (Tiles / 2) + 2
		3'b001: MaxValueTileMap2BeRead = 8'd22; 	// (52) 16 + 640 + 16 ( 1 + 40 + 1) Y: 600 + 32 = 40 tiles
		3'b010: MaxValueTileMap2BeRead = 8'd27;	// (66) 16 + 800 + 16 ( 1 + 50 + 1) Y: 768 + 32 = 15 Tiles
		3'b011: MaxValueTileMap2BeRead = 8'd27; 	// (42) 16 + 800 + 16 ( 1 + 50 + 1) Y: 400 + 32 = 21 Tiles
	
		3'b100: MaxValueTileMap2BeRead = 8'd12;	// (22) 16 + 320 + 16 ( 1 + 20 + 1) Y: 240 + 32 = 15 Tiles
		3'b101: MaxValueTileMap2BeRead = 8'd12; 	// (27) 16 + 400 + 16 ( 1 + 20 + 1) Y: 300 + 32 = 21 Tiles
		3'b110: MaxValueTileMap2BeRead = 8'd15;	// (22) 16 + 512 + 16 ( 1 + 25 + 1) Y: 384 + 32 = 15 Tiles
		3'b111: MaxValueTileMap2BeRead = 8'd15; 	// (22) 16 + 320 + 16 ( 1 + 25 + 1) Y: 200 + 32 = 15 Tiles
	endcase
end
...
// This is to have a Relative Position of the Border that will serve for the Scroll 
//assign TileMap_X_Window_Pos_Compute_Minus1 = (TileMap_X_Window_Pos == 10'b00_0000_0000) ? 10'b00_0000_0000 : TileMap_X_Window_Pos - 10'b00_0000_0001;

//assign TileMap_Y_Window_Pos_Compute = TileLineChoice ? ((TileMap_Y_Window_Pos == 10'b00_0000_0000) ? 10'b00_0000_0000 : TileMap_Y_Window_Pos - 10'b00_0000_0001) : TileMap_Y_Window_Pos;




always @ (posedge EngineClk100Mhz_i) begin
	//TileMap_X_Window_Pos_Compute_Minus1 <= (TileMap_X_Window_Pos == 10'b00_0000_0000) ? 10'b00_0000_0000 : TileMap_X_Window_Pos - 10'b00_0000_0001;
	//TileMap_X_Window_Pos_Compute_Minus1 <= (TileMap_X_Window_Pos == 10'b00_0000_0000) ? 10'b00_0000_0000 : TileMap_X_Window_Pos;
...
DMA_MULT_BLK TileMapAddyLineCompute(
	...
	// THis is the Effective Address of the upper corner of the Window including the Scroll Border
//assign TileMap_Effective_Addy	= Addy_2_Compute[21:0] + Absolute_Compute_Mult[21:0] + {12'b0000_0000_0000, TileMap_X_Window_Pos_Compute_Minus1};	// Tile MAP Addy
//assign VGE_TileMap_Addy 		= TileMap_Effective_Addy + Relative_Compute_Mult[21:0] + {14'b0000_0000_0000_0000, Buffer_X_Pointer, 2'b00};

wire	[31:0] ADDY_ADDER_Results;

//assign TileMap_X_Window_Pos_Compute_Plus1 = TileMap_X_Window_Pos[9:0] + 10'b00_0000_0001;

ADDY_ADDER	ADDY_ADDER_inst (
	.data0x ( {10'b00_0000_0000, Addy_2_Compute[21:0]} ),
	.data1x ( {10'b00_0000_0000, Absolute_Compute_Mult[21:0]} ),
	//.data2x ( {21'b00_0000_0000_0000_0000_0000, TileMap_X_Window_Pos_Compute_Minus1, 1'b0} ),
...
always @ (posedge EngineClk100Mhz_i) begin
	...
	//(Time_2_Charge_TileMap_Lines_i[1:0] == 2'b00)
	//			if (( TileMap0Enabled_i | TileMap1Enabled_i | TileMap2Enabled_i | TileMap3Enabled_i ) && ( TileMap_Effect_On_i )) begin
		case (VGE_TileMap_SM)
		
		// Let's wait for the Cue to go Fetch
		// Lets
		TLMAP_IDLE: begin
			if ( Time_2_Charge_TileMap_Lines_i[0]  &&  TileMap_Effect_On_i  && Trig_TL_Read_Memory_i) begin	// 2'b11 Represent the beginning of a Frame - So Precharge 2 lines
				TileMap_Actual_Layer 	<= 2'b00;			// The Layer Counter
				MAP_DATA						<= 1'b1;				
				VGE_TileMap_SM 			<= TLMAP_PREP0;

				if ( Time_2_Charge_TileMap_Lines_i[1] ) begin
		...
		TLMAP_PREP1: begin
			if (TileMapEnabled_Channel) begin
		...
		TLMAP_WAIT_END: begin
			if ( Time_2_Charge_TileMap_Lines_i[0]  &&  TileMap_Effect_On_i )		
				VGE_TileMap_SM		<= TLMAP_WAIT_END;
			else
				VGE_TileMap_SM		<= TL_IDLE;
		end
...
//assign Trigger = (Time_2_Charge_TileMap_Lines_i[1:0] == 2'b11) & TileMap_Effect_On_i & Trig_TL_Read_Memory_i;
...
assign ChipScope[46:45] 	= Time_2_Charge_TileMap_Lines_i;
...
assign Tile2Pick_Addy = { TileBank, TileLinePointer[4], Tile2Pick_Addy_Position };
...
always @ (*) begin
	case({ Mstr_Ctrl_Doubling_Pixel_100Mhz_i, Mstr_Ctrl_Video_Mode100Mhz_i[1:0]})
		3'b000: NumberOfTile2Read = 8'd44;	// (42) 16 + 640 + 16 	( 1 + 40 + 1)
		3'b001: NumberOfTile2Read = 8'd44;  // (52) 16 + 640 + 16 	( 1 + 40 + 1)
		3'b010: NumberOfTile2Read = 8'd54; 	// (66) 16 + 800 + 16 	( 1 + 54 + 1)
		3'b011: NumberOfTile2Read = 8'd54; 	// (27) 16 + 800 + 16 	( 1 + 54 + 1)
		
		// Doubling Mode
		3'b100: NumberOfTile2Read = 8'd24;	// (42) 16 + 320 + 16 	( 1 + 20 + 1)
		3'b101: NumberOfTile2Read = 8'd24;  // (52) 16 + 400 + 16 	( 1 + 25 + 1)
		3'b110: NumberOfTile2Read = 8'd29; 	// (22) 16 + 512 + 16 	( 1 + 32 + 1)
		3'b111: NumberOfTile2Read = 8'd29; 	// (27) 16 + 320 + 16 	( 1 + 20 + 1) 
	endcase
end					
...
assign TileData_Addy_Stop 	= Active_TileSet_Addy[21:2] + {6'b00_0000, Pixel2FetchCounter_TL[15:2]} + 20'h0_0004;


always @ (*)
begin
	if (TileStride256x256) begin
		Pixel2FetchCounter_TL = {Active_Tile_Data[7:4], TileLinePointer[3:0], Active_Tile_Data[3:0], 4'b0000};
	end
	else begin
		Pixel2FetchCounter_TL = {Active_Tile_Data[7:0], TileLinePointer[3:0], 4'b0000};
	end
end
	...
	if (VGE_Engine_Rst_i || Reset_100Mhz_i) begin
		...
		TL_STATE0: begin
			if (Tile_Layer_Control_Reg[0])		// Check if the layer is On
				...
				//Clear_PixelLine_TileMap		<= 1'b1;
				//VGE_Tile_Engine_SM <= TL_CLEAN_LINE0;	
				//VGE_Tile_Engine_SM_SM   <= TL_STATE8;	
		...
		if (Tile_Y_Scroll_Reg[TileBank][4]) 
...
// Tile
// This Code Will Calcule the Address for the VRAM.
/*
always @ (posedge EngineClk100Mhz_i) 
begin
	if (VGE_Engine_Rst_i) begin
		...
		if (VGE_TileData_Readn) begin
			...
			Pixel2FetchCounter_TL <= Pixel2FetchCounter_TL + 16'b0000_0000_0000_0100;
	end
end
*/
...
always @ (posedge EngineClk100Mhz_i) begin
	if (VGE_Engine_Rst_i) begin
		Addy_2_Compute 			<= 22'h00_0000;
		TileMap_X_Size_Compute 	<= 10'h0_00;
		TileMap_X_Window_Pos 	<= 10'h0_00;
		TileMap_Y_Window_Pos 	<= 10'h0_00;
		TileMapEnabled_Channel 	<= 1'b0;
		TileLineChoice				<= 1'b0;
	end
	else begin
			case(TileMap_Actual_Layer)
			2'b00: begin
				...
				TileMap_Y_Window_Pos 	<= TileMap0_Y_Window_Pos_i;
				TileMapEnabled_Channel 	<= Tile0_Layer_Control_Reg_i[0];
				TileLineChoice 			<= Tile0_Y_Scroll_Reg_i[4];
			end
			2'b01: begin
				Addy_2_Compute 			<= TileMap1Addy_i;
				TileMap_X_Size_Compute 	<= TileMap1_X_TotalSize_i;
				...
				TileMap_Y_Window_Pos 	<= TileMap1_Y_Window_Pos_i;
				TileMapEnabled_Channel 	<= Tile1_Layer_Control_Reg_i[0];
				TileLineChoice 			<= Tile1_Y_Scroll_Reg_i[4];
			end
			2'b10: begin
				Addy_2_Compute 			<= TileMap2Addy_i;
				TileMap_X_Size_Compute 	<= TileMap2_X_TotalSize_i;
				...
				TileMap_Y_Window_Pos 	<= TileMap2_Y_Window_Pos_i;
				TileMapEnabled_Channel 	<= Tile2_Layer_Control_Reg_i[0];
				TileLineChoice 			<= Tile2_Y_Scroll_Reg_i[4];	
			end
			2'b11: begin
				Addy_2_Compute 			<= TileMap3Addy_i;
				TileMap_X_Size_Compute 	<= TileMap3_X_TotalSize_i;
				...
				TileMap_Y_Window_Pos 	<= TileMap3_Y_Window_Pos_i;
				TileMapEnabled_Channel 	<= Tile3_Layer_Control_Reg_i[0];
				TileLineChoice 			<= Tile3_Y_Scroll_Reg_i[4];
			end
		endcase
	end
end
