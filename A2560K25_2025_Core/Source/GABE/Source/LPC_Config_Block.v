...
*/
		case (StateMachine)
			...
			IDLE: begin
				ResetCounter	<= 24'h325AA0;
				PCI_Reset				<= 1'b0;
				Config_Done_o			<= 1'b0;				
				StateMachine 	<= RESET_LOW;
			end
					...
					INT0: begin
						...
						if (CONFIG[7:0] == 8'h07)
							...
							if (CONFIG[7:0] == 8'h03) begin
								...
								if (CONFIG[7:0] == 8'h00) begin
									Logic_Device_State <= FDC;								
								end
								else begin
			...
			// RESET The LPC before config
			RESET_LOW: begin
				if (ResetCounter)
					ResetCounter <= ResetCounter - 24'h000001;
				else begin
					PCI_Reset				<= 1'b1;
					ResetCounter	<= 24'hFC0000;
					StateMachine 	<= RESET_HIGH;
				end
			end
