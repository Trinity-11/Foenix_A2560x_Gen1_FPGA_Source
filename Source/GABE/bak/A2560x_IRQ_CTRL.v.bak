...
assign TinyTrigger = LPC_IRQ_i[1];

assign TinyTP[31:0]  	= irq_o_RESYNC[2];
assign TinyTP[38:32] 	= iIRQ_Interrupt_o;			// Level
assign TinyTP[39]   		= iIRQ_AutoVector_o;			// Auto Vector
assign TinyTP[47:40]   	= iIRQ_Vector_o;				// Vector
assign TinyTP[48]   		= A2560K_Keyboard_IRQ_i;
assign TinyTP[49]   		= Ethernet_IRQ_i;
assign TinyTP[50]   		= RTC_IRQ_i;
assign TinyTP[51]   		= IDE_IRQ_i;
assign TinyTP[52]   		= SD_IRQ_i;
assign TinyTP[53]   		= SD_Card_Insert_i;
assign TinyTP[54]   		= Timer0_i;						// CPU Clock Timer 0
assign TinyTP[55]   		= Timer1_i;						// CPU Clock Timer 1
assign TinyTP[56]   		= Timer2_i;						// CPU Clock Timer 2
assign TinyTP[57]   		= Timer3_i;						// SOF Channel A Counter IRQ
assign TinyTP[58]   		= Timer4_i;						// SOF Channel B Counter IRQ
assign TinyTP[59]   		= Interrupt_Group[0];
assign TinyTP[60]   		= Interrupt_Group[1];
assign TinyTP[61]   		= Interrupt_Group[2];
assign TinyTP[62]   		= Interrupt_Group[3];
assign TinyTP[63]   		= Interrupt_Group[4];
assign TinyTP[64]   		= Interrupt_Group[5];
assign TinyTP[117:70]	= pending;
assign TinyTP[130:118]  = statemachine_Debug_RESYNC[2];
assign TinyTP[131] 		= serirq_i_RESYNC[2];
assign TinyTP[143:132]	= 0;


TinyChipScope InterruptCS (
...
/*
//wire	[31:0]	LPC_IRQ_i,
reg [2:0] Reset_LPC_IRQ;
always @ ( posedge LPC_Clk_i ) begin
		Reset_LPC_IRQ[0] <= !RST_i;
		Reset_LPC_IRQ[1] <= Reset_LPC_IRQ[0];
		if ( Reset_LPC_IRQ[1] == Reset_LPC_IRQ[0])
			Reset_LPC_IRQ[2] <= Reset_LPC_IRQ[1];
end
*/
...
serirq_host LPC_SerialIRQ_Block(
   ...
   .serirq_mode_i( 1'b0 ), 			// Quiet mode?
	.irq_o( irq_o ),
	.serirq_i(serirq_i),
   .serirq_o(serirq_o), 
	.serirq_oe( serirq_oe ),
	.statemachine_Debug( statemachine_Debug  ) 
);
wire [12:0] statemachine_Debug;


// Debug 
reg [2:0]		 serirq_i_RESYNC;
reg[12:0] statemachine_Debug_RESYNC[0:2];

// RESYNC from LPC Clock Domain to CPU Clock Domain 
always @ (posedge CPU_Clk_i ) begin

	serirq_i_RESYNC[0] <= serirq_i;
	serirq_i_RESYNC[1] <= serirq_i_RESYNC[0];
	if ( serirq_i_RESYNC[1] == serirq_i_RESYNC[0] )
		serirq_i_RESYNC[2] <= serirq_i_RESYNC[1];

	statemachine_Debug_RESYNC[0] <= statemachine_Debug;
	statemachine_Debug_RESYNC[1] <= statemachine_Debug_RESYNC[0];
	if ( statemachine_Debug_RESYNC[1] == statemachine_Debug_RESYNC[0] ) begin
		statemachine_Debug_RESYNC[2] <= statemachine_Debug_RESYNC[1];
	end 
end 
...
/*
extern void Level_1_AutoVector_Srv( void );	// SOF Channel B 	(Graphic Side)
extern void Level_2_AutoVector_Srv( void );	// Vicky III Channel B (Graphic Side)
extern void Level_3_AutoVector_Srv( void );	// SOF Channel A 	(Text Side)
extern void Level_4_AutoVector_Srv( void );	// Vicky III Channel A (Text Side)
extern void Level_5_AutoVector_Srv( void );	// GABE Interrupt (SuperIO/Timer/Etc...)
extern void Level_6_AutoVector_Srv( void );	// BEATRIX Interrupt (DAC FIFO, Sound Chip Interrupt, Etc...)
extern void Level_7_AutoVector_Srv( void );	// NMI - TBD
Interrupt
...
1B -  BTX_IRQ_i
1C - TBD
1D - DAC1_Playback_Done_IRQ_i
1E - TBD
1F - DAC0_Playback_Done_IRQ_i
*/
...
always @(posedge CPU_Clk_i) begin
	lirq0 <= {  DAC0_Playback_Done_IRQ_i, 1'b0, DAC1_Playback_Done_IRQ_i, 1'b0, BTX_IRQ_i[3:0], 2'b00, OPL3_EXT_IRQ_i, OPN2_EXT_IRQ_i, OPM_IXT_IRQ_i, SD_IRQ_i, SD_Card_Insert_i, IDE_IRQ_i,
					RTC_int_PulSe[3], ETH_int_PulSe[3], 1'b0, ~Timer4_i, ~Timer3_i, ~Timer2_i, ~Timer1_i, ~Timer0_i, !MPU_401_int_PulSe[3], !FDC_int_PulSe[3], !LPT1_int_PulSe[3], !COM2_int_PulSe[3], !COM1_int_PulSe[3], !Mouse_int_PulSe[3], !A2560Keyboard_int_PulSe[3] , !Keyboard_int_PulSe[3], //16bits
...
/*
00 - SOF - VICKY CHANNEL A - INT0
01 - SOL - VICKY CHANNEL A - INT1
02 - VICKY CHANNEL A - INT2
03 - VICKY CHANNEL A - INT3
04 - VICKY CHANNEL A - INT4
05 - VICKY CHANNEL A - INT5
06 - RESERVED
07 - VIDEO DAC CHANNEL A - Hot Plug

08 - SOF - VICKY CHANNEL B - INT0
09 - SOL - VICKY CHANNEL B - INT1
0A - VICKY CHANNEL B - INT2
0B - VICKY CHANNEL B - INT3
0C - VICKY CHANNEL B - INT4
0D - VICKY CHANNEL B - INT5
0E - RESERVED
0F - VIDEO DAC CHANNEL B - Hot Plug
...
assign TinyTrigger1 = CS_Interrupt_Ctrl_i & ( {CPU_A_Valid_EDGE, CPU_A_Valid_i} == 2'b10 );

assign TinyTP1[23:0]  	= CPU_A_i;
assign TinyTP1[39:24] 	= CPU_D16_i;
assign TinyTP1[40] 		= CPU_RW_i;
assign TinyTP1[42:41] 	= CPU_Siz_i[1:0];
assign TinyTP1[44:43]	= {CPU_A_Valid_EDGE, CPU_A_Valid_i};
