...
output	reg				Dbg_CPU_Rst_o, /* synthesis preserve noprune */
output	wire				Dbg_RW_o,

output	wire				Dbg_RAM_CS_Lo_o,
output	wire				Dbg_RAM_CS_Hi_o,
output	wire				Dbg_Flash_CS_Lo_o,
output	wire				Dbg_Flash_CS_Hi_o,
...
//assign TinyTrigger = (Dbg_A_o == 32'h00000400);
//assign TinyTrigger = DebugDebug_o;

assign TinyTP[31:0]  	= Dbg_A_o;
assign TinyTP[63:32] 	= Dbg_D_o;
assign TinyTP[95:64]   	= Dbg_D_i;
// RAM
assign TinyTP[96]   		= Dbg_RAM_CS_Lo_o;
assign TinyTP[97]   		= Dbg_RAM_CS_Hi_o;
assign TinyTP[98]   		= Dbg_Flash_CS_Lo_o;
assign TinyTP[99]   		= Dbg_Flash_CS_Hi_o;

assign TinyTP[100]		= Dbg_RW_o;
assign TinyTP[101]   	= Dbg_Oe_o;
assign TinyTP[102]		= Dbg_CPU_Rst_o;
assign TinyTP[103]		= Dbg_Mode_o;
...
localparam DEBUG_REV = 8'h02;				// In the Future, there will be probably some different Support from the Debugger
...
assign Dbg_RAM_CS_Lo_o 	= Flash_Program_Mode ? Flash_Program_RAM_CSn_LO : ((Dbg_A_o[31:22]  == 10'b0000_0000_00) & CS_Trig); 	// $00:00000 - $3F:FFFF - 2Meg		// User RAM
assign Dbg_RAM_CS_Hi_o 	= Flash_Program_Mode ? Flash_Program_RAM_CSn_HI : ((Dbg_A_o[31:22]  == 10'b0000_0000_00) & CS_Trig);	// $00:00000 - $3F:FFFF - 2Meg		// Expansion RAM
				
assign Dbg_Flash_CS_Hi_o 	= Flash_Program_Mode ? Flash_Program_FLASH_CS_HI : ((Dbg_A_o[31:22]  == 10'b1111_1111_11) & CS_Trig); 	// $FFC0:00000 - $FFFF:FFFF - 4Meg		// System FLASH/7			
assign Dbg_Flash_CS_Lo_o 	= Flash_Program_Mode ? Flash_Program_FLASH_CS_LO : ((Dbg_A_o[31:22]  == 10'b1111_1111_11) & CS_Trig); 	// $FFC0:00000 - $FFFF:FFFF - 4Meg		// System FLASH/7
// CPU SIDE
assign Dbg_A_o =  Flash_Program_Mode ? Flash_Program_Addy : CPU_Address;
assign Dbg_D_o  = Flash_Program_Mode ? Flash_Program_Data   : { CPU_Data_Out[7:0], CPU_Data_Out[15:8], CPU_Data_Out[23:16], CPU_Data_Out[31:24]};
assign Dbg_RW_o = Flash_Program_Mode ? Flash_Program_RD_WRn : CPU_R_Wn;
...
wire				Flash_Program_FLASH_CS_LO;		// Flash
wire				Flash_Program_FLASH_CS_HI;		// Flash
wire				Flash_Program_RAM_OEn;
wire				Flash_Program_RAM_CSn_LO;
wire				Flash_Program_RAM_CSn_HI;
...
// RAM Interface
	...
	.Flash_CS_Hi_o( Flash_Program_FLASH_CS_HI ),					// Flash Chip Select	
	.Flash_CS_Lo_o( Flash_Program_FLASH_CS_LO ), 

	.RAM_CSn_Lo_o( Flash_Program_RAM_CSn_LO ),				// RAM Chip Select 0
	.RAM_CSn_Hi_o( Flash_Program_RAM_CSn_HI ),				// RAm Chip Select 1	
...
Debug_Serial_Buffer DEBUG_SERIAL_BUF(
	...
	.address_a( Dbg_DP_Pointer[10:0] ),
	.data_a( {Dbg_D_i[7:0], Dbg_D_i[15:8], Dbg_D_i[23:16], Dbg_D_i[31:24]}  ),
	.wren_a( Dbg_DP_Wren ),
	.q_a( CPU_Data_Out )
);
...
begin
	if (System_Rst_i) begin
		DBG_SM 			<= DBG_IDLE;
		Dbg_CPU_Rst_o  <= 1'b1;	// This is to Reset the 68SEC000 
//		Dbg_CPU_Halt_o <= 1'b1;	// this is to Halt the 68SEC000
		Dbg_DP_Wren		<= 1'b0;
		Dbg_Mode_o		<= 1'b0;
		CS_Trig		   <= 1'b0;
		CPU_R_Wn			<= 1'b1;
	...
	//					DBG_SMSM <= DBG_CMD2;
			...
			DBG_READ1: begin
				if (Dbg_DP_Pointer < ({2'b00, Dbg_CMD_FIFO_Out[39:26]}) + (  Dbg_CMD_FIFO_Out[25:24] ? 16'h0001 : 16'h0000 ) )	// Change for MC68040 Divide the Byte/4 for we store INTs
				begin
					Dbg_DP_Pointer 	<= Dbg_DP_Pointer + 16'h0001;				
					CPU_Address			<= CPU_Address    + 32'h0000_0004; // Address = Address + 2 (we are in bytes)
					DBG_SM 				<= DBG_READ1;
				end
				else begin
					CPU_Oe		<= 1'b1;
			...
			// Write is Valid Here
			DBG_WRITE2: begin		
				CPU_Address				<= CPU_Address    + 32'h0000_0004;
				DBG_SM 					<= DBG_WRITE3;
			end
			
			DBG_WRITE3: begin
				if (Dbg_DP_Pointer < {2'b00, Dbg_CMD_FIFO_Out[39:26]}) // This is the Count in Bytes (but divide 2)
				begin
					CPU_R_Wn			<= 1'b0;				
					DBG_SM 				<= DBG_WRITE1;
				end
			...
			DBG_STOP_CPU0: begin
				...
				Dbg_CPU_Rst_o	<= 1'b0;		// Now that we are going to take over the bus, let's first Reset the CPU. Later we will try to only stop it.
//				Dbg_CPU_Halt_o <= 1'b0;		// To do a Full Reset with the MC68SEC000 you need to pull down the Reset and Halt
			...
			DBG_RESTART1: begin
				Dbg_Cmd_Ack_Stat 	<= Dbg_Cmd_Ack_Stat | DbgAckStatus_Exe_Done;				
				Dbg_CPU_Rst_o		<= 1'b1;				// Now that we are going to take over the bus, let's first Reset the CPU. Later we will try to only stop it.			
//				Dbg_CPU_Halt_o 	<= 1'b1;		// To do a Full Reset with the MC68SEC000 you need to pull down the Reset and Halt				
				DBG_SM 				<= DBG_EXEC_ACK0;	// Then, Go and Acknowledged that the Command was Executed
			end
...
localparam		CMD_RD_MEM_BLK = 8'b0000_0000,		// This corresponds to RAM and or FLASH
					CMD_WR_MEM_BLK	= 8'b0000_0001,		// This corresponds to RAM and or FLASH
					CMD_RD_REG_LOC = 8'b0000_0010,		// Go Read 1 Register Only
					CMD_WR_REG_LOC = 8'b0000_0011,		// Go Write 1 Register Only
					CMD_PROG_FLASH = 8'b0001_0000,		// Setup the Flash Programming parameters
					CMD_ERASE_FLSH = 8'b0001_0001,		// Execute the Write Procedure - 
					CMD_PRG_BRK		= 8'b0010_0000,		// Setup BreakPoint Controller (Stops the CPU by Bring RDY low and poking around)
					CMD_PRG_BRKPT	= 8'b0010_0001,		// Setup BreakPoints
					CMD_FILL_MEM	= 8'b0100_0000,		// FILL a memory region with a Byte
					CMD_TSF_MEM		= 8'b0100_0001,		// Transfer a Block of Memory From Src to Dst
					CMD_TSF_MEMCPX = 8'b0100_0010,		// Transfer a Block of Memory From Src to Dst Complexed (with Stride)
					CMD_STOP_CPU   = 8'b1000_0000,		// This needs to be done before doing anything else
					CMD_START_CPU  = 8'b1000_0001,		// This needs to be done after everything has been done
					CMD_RLSE_RDY	= 8'b1000_0010,		// Release RDY created by a BreakPoint
					CMD_GET_REV		= 8'b1111_1110,		// Get the Revision of the Debug Block
					CMD_GET_STATUS = 8'b1111_1111;		// Get the Status
