...
begin
	if (Reset_VideoClk_Full_Resolution) begin
		...
		if ( Mstr_Ctrl_Video_Mode_ReSync[0] == Mstr_Ctrl_Video_Mode_ReSync[1] )
		...
		if ( Mstr_Ctrl_Doubling_Pixel_ReSync[0] == Mstr_Ctrl_Doubling_Pixel_ReSync[1] )
...
//	if (Reset_VideoClk_Full_Resolution || reset_mode) begin
	if ( Reset_VideoClk_Full_Resolution ) begin	
		...
		if (Mstr_Ctrl_Video_Mode_ReSync[2]) begin
...
begin
	if (Reset_VideoClk_Full_Resolution || reset_mode) begin
		...
		if (VGE_Engine_VBlanking_o)  begin
			if (	HPixelCount_Local == { 1'b0 HBLANK_STOP[11:1],1'b0} )
			Visible_Local_Line_Counter <= Visible_Local_Line_Counter + 11'd1;
		end
		else begin
			Visible_Local_Line_Counter		<= 11'b00_0000_0000;
		end
...
begin
	if (Reset_VideoClk_Full_Resolution || reset_mode) begin
				...
				Charge_TL1_Tile_Lines <= 2'b00;
				Charge_TL2_Tile_Lines <= 2'b00;
				Charge_TL3_Tile_Lines <= 2'b00;
	end
	else begin
		// Tile Layer 0
		if ( VGE_Engine_VBlanking_o && TL0Condition2Trigger ) begin
			if (HPixelCount_Local < HPixelCount_Target)
		...
		// Tile Layer 1
		if ( VGE_Engine_VBlanking_o && TL1Condition2Trigger ) begin
			if (HPixelCount_Local < HPixelCount_Target)
		...
		// Tile Layer 2
		if ( VGE_Engine_VBlanking_o && TL2Condition2Trigger ) begin
			if (HPixelCount_Local < HPixelCount_Target)
		...
		// Tile Layer 2
		if ( VGE_Engine_VBlanking_o && TL3Condition2Trigger ) begin
			if (HPixelCount_Local < HPixelCount_Target)
		...
		// First line
		if (VGE_Engine_VBlanking_o && (Visible_Local_Line_Counter == 11'h000)) begin
...
/////////////// THIS CODE IS NOT UP TO DATA --- SEE VIII_VideoGen_B_NEW for latest Version /////////////////
...
// 0 -> 1799 for 1290x960 (1800 Pixels in 1 Line) (Starts @ 519)
// 0 -> 1687 for 1280x1024 (1688 Pixels in 1 Line) (Starts @ 407)
// Remember the Clock is running @ 108Mhz
always @ (posedge VideoClk_i) 
begin
	if (Reset_VideoClk_Full_Resolution || reset_mode) begin
		...
		if ((HPixelCount_Local == 12'd1799) && VGE_Engine_VBlanking_o && !Mstr_Ctrl_Video_Mode_i[0])
					Time_Line_2_LUT_2_FinalLine <= 4'b1111;					//<<<<<<<<
			
		if ((HPixelCount_Local == 12'd1687) && VGE_Engine_VBlanking_o && Mstr_Ctrl_Video_Mode_i[0])
					Time_Line_2_LUT_2_FinalLine <= 4'b1111;					//<<<<<<<<			
	end
end
