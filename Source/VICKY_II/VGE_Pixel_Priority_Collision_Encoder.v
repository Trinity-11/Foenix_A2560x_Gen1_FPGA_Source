...
// 1024 x32 with 2 Latency
Final_RGB_Pixel_Line OUTPUT_RGB(

	.rdaddress( Mstr_Ctrl_Doubling_Pixel_i ? {1'b0, Video_Pixel_Pointer[10:2]} : {Video_Pixel_Pointer[10:1]}),
	.rdclock( VideoClk_i ),
	.q( VGE_RGB_Pixel_o ),
...
// Now because the Video Clock is 108Mhz, the Counter will be divided later 2 or by 4
// By default in Game Engine Mode, the Resolution is either 640x480/320x240 or 640x512/320x256
always @ (posedge VideoClk_i)
begin
	if ( Reset_VideoClkOut_i || VideoModeReset_i ) begin
		Video_Pixel_Pointer <= 11'b000_0000_0000;	
	end
	else begin
		//if (Time_2_Display_Line_VidClk_i) begin
		if (HBlanking_VGE_Lat_i && VBlanking_i) begin
			Video_Pixel_Pointer <= Video_Pixel_Pointer + 11'b000_0000_0001;
		end
		else begin
			Video_Pixel_Pointer <= 11'd32;
		end
	end
end
